...
import bonus.sources.Pictures;

import java.awt.Color;
import java.util.ArrayList;

public class Mandelbrote extends Thread {
    final static int taille = 500 ;   // nombre de pixels par ligne et par colonne
    final static Pictures image = new Pictures(taille, taille) ;
    ...
    public Mandelbrote(int numThread) {
        this.numThread = numThread;
    }
    
    public static void main(String[] args)  {

        ArrayList<Mandelbrote> threadList = new ArrayList<>();

        final long début = System.nanoTime() ;

        for (int i = 1 ; i <= nbThread; i++){
            Mandelbrote mandelbrote = new Mandelbrote(i);
            threadList.add(mandelbrote);
            mandelbrote.start();
        }



        for (Mandelbrote m : threadList) {
            try {
                m.join();
        ...
        System.out.println("Durée total = " + (double) durée / 1000 + " s.") ;
    ...
    public void run(){
        ...
        for (int i = 0; i < taille; i++) {
            for (int j = (numThread-1) * (taille/nbThread); j < (taille/nbThread)*numThread; j++) {
                colorierPixel(i,j) ;
            }
            synchronized (image) {
        ...
        final long fin = System.nanoTime() ;
        final long durée = (fin - début) / 1_000_000 ;
        System.out.println("thread " + this.numThread+ ": Durée = " + (double) durée / 1000 + " s.") ;
    }
}
